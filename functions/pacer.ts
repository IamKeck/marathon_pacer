...
const handler: Handler = async (event, context: HandlerContext) => {
    ...
    const distance = parseInt(match[1] || "0", 10);
    ...
    const [targetHours, targetMinutes, targetSeconds] = time
        .split("_")
        .map(Number);
    if (targetHours === undefined || targetMinutes === undefined || targetSeconds === undefined) {
        return {
            statusCode: 400,
            body: JSON.stringify({error: "Invalid time format"}),
        };
    }
